...
/*
    Dino Runner (콘솔, Windows 전용)
    - LOW 장애물: 선인장 '|' (지면 포함 위쪽, 고정 높이)
    - HIGH 장애물: 새 "<^>" (groundY-1) → 숙여서 피함
    - 장애물 간 최소 간격 유지
    - 속도: 게임 내내 일정 (레벨 시스템 제거)
    - 코인: 점수 400마다 +1
*/
...
static void initGame(const Config& cfg, State& st) {
    st.y = cfg.groundY;
    st.vy = 0.0;
    st.onGround = true;
    st.duck = false;
...
static void handleInput(const Config& cfg, State& st) {
    if (_kbhit()) {
        int ch = _getch();
...
static void maybeSpawnObstacle(const Config& cfg, State& st) {
...
static void tick(const Config& cfg, State& st) {
    ...
    // 최고점 갱신은 게임오버 때만 반영(원하는 경우 여기서도 가능)
}

static void draw(const Config& cfg, const State& st) {
    moveHome();
...
static void startGame(const Config& cfg, State& st) {
    while (true) {
        initGame(cfg, st);
